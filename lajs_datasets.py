...
class SegmentSelectionDataset(object):
    def __init__(self, config, data_file=None):
        ...
        self.chunk_size = config['chunk_size']          # 窗口大小
        self.config = config
    ...
    def chunk(self, text):  ### 窗口不重叠
        chunks_tokens = []
        tokens = self.tokenizer.tokenize(text)     #text为待分块内容
        num = len(tokens) // self.chunk_size       #num为分成多少块数
        if num * self.chunk_size < len(tokens):    #检查最后一块不满254字的块有无计入，无则加一
            num += 1
        for i in range(num):                       #开始分块以每块254个字符为一块，例chunks_tokens.append(tokens[0:254])
            chunks_tokens.append(tokens[i * self.chunk_size:(i + 1) * self.chunk_size])
        return chunks_tokens                       #返回一个list
    ...
    def chunk_overlap(self, text, window_size=254, overlap=127, cutoff=True):  ### 窗口重叠  64
        ...
        while start < len(tokens):
            window_tokens = tokens[start:start+window_size]         #取前254个字符
            if cutoff:
                if len(window_tokens) > 20:
                    chunks_tokens.append(window_tokens)             #加入chunks_tokens
            else:
                chunks_tokens.append(window_tokens)
            start += overlap                                        #start加上127，即偏移127字符重新截取进chunks_tokens
    ...
    def encode(self, q_crime, d_crime, q, d, q_chunksize=100, d_chunksize=100, cutoff=True):  # q_chunksize=3, d_chunksize=15 for train
        ids, type_ids = [], []
        q_crime_tokens, d_crime_tokens = self.tokenizer.tokenize(q_crime), self.tokenizer.tokenize(d_crime)   #取罪名
        crime_tokens = ['[CLS]'] + q_crime_tokens + ['[SEP]'] + d_crime_tokens + ['[SEP]']                    #拼接
        ...
        q_chunks, d_chunks = self.chunk(q), self.chunk_overlap(d, cutoff=cutoff)           #截取字段
        q_chunks, d_chunks = q_chunks[:q_chunksize], d_chunks[:d_chunksize]                #各取前100个案例
        for q_chunk in q_chunks:
            for d_chunk in d_chunks:
                tokens = ['[CLS]'] + q_chunk + ['[SEP]'] + d_chunk + ['[SEP]']             #将每个q段和d段按格式拼接在一起
                token_ids = self.tokenizer.convert_tokens_to_ids(tokens)
                types = [0] * (len(q_chunk) + 2) + [1] * (len(d_chunk) + 1)
                ids.append(token_ids)                                                      #标顺序
    ...
    def __getitem__(self, index):
        ...
        docs = random.sample(sample_docs, k=self.config['random_k'])                       #随机不重复取sample_docs内的4个
    ...
    def get_predict_samples(self, q, q_crime, d):
        d_crime, content = d['d_crime'], d['content']                                   #获取罪名与基本情况
