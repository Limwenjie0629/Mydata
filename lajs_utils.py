...
def cut_doc(doc):
    return [s for s in re.split(r'[。！；？]', doc) if len(s) > 0]                 #分句


def get_stopwords(stw_file):                                                     #停止词
...
def search_for_related_sents(query, sents_list, stopwords=None, select=1):
    ...
    for sent in sents_list:
        sent_tokens = [w for w in jieba.lcut(sent) if w not in stopwords]        #候选集句子分句
        corpus.append(sent_tokens)
    bm25model = bm25.BM25(corpus)                                                #使用bm25计算句子相似度

    q_tokens = [w for w in jieba.lcut(query) if w not in stopwords]              #quary句子分句
    scores = bm25model.get_scores(q_tokens)
    rank_index = np.array(scores).argsort().tolist()[::-1]
    rank_index = rank_index[:select]
    return [sents_list[i] for i in rank_index], [scores[i] for i in rank_index]  #返回得分


def select_relavence(query_file, candidate_dir, stw_file, saved_file, sent_group=5, select=1):
    queries = []
    stopwords = get_stopwords(stw_file)                                          #获取停止词
    with open(query_file, 'r', encoding='utf8') as f:
        for line in f:
            queries.append(json.loads(line.strip()))

    data = []
    for query in queries:                                                        #取quary数据集
        ...
        selected_docs = []
        for doc_file in doc_files:                                               #取候选集
            ...
            sents_list = cut_doc(doc)                                            #对候选集分句
            pre_sents = sents_list[:sent_group*2]                                #取前10个句子
            ...
            group_sents = []
            for i in range(0, len(sents_list), sent_group // 2):                 #取除了前10的句子
            ...
            rel_sents, scores = search_for_related_sents(q, group_sents, stopwords, select=select)  #选出最相似的句子
